/**
 * dsh-balance 启动壳验证（无外部依赖，不碰真实 ~/.dsh）。
 *
...
const root = mkdtempSync(join(tmpdir(), 'dsh-balance-shell-'))
...
`)
  ...
  check('host.js 的 inject 原样传给 cordis', Array.isArray(mounts[0]?.inject) && mounts[0].inject[0] === 'webServer')
